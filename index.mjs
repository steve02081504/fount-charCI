...
const CHAR_DIRECTORY = fs.realpathSync(`./fount/data/users/${username}/chars/${charname}`)
const EMOJI = {
	test: '🧪',
	success: '✅',
	fail: '❌',
...
function getContext(name) {
	...
	const console = new VirtualConsole({
		...
		error_handler: (error) => {
			...
			const relativePath = error.fileName.replace(CHAR_DIRECTORY + '/', '')
...
async function runTest(testName, testFunction, {
	...
	try {
		...
		if (!is_top_level) {
			...
			parentContext.console.log(testConsole.outputs)
		}
...
if (char?.interfaces.config && char?.interfaces.chat) {
	...
	CI.runInput = async (input, request) => {
		if (typeof input === 'string') input = { role: 'user', content: input, files: [] }
...
await CI.test('CI Main', async () => {
	try {
		// 动态导入命令行参数指定的测试文件
		await import(path.resolve(path.join(CHAR_DIRECTORY, process.argv[2])))
	} catch (error) {
		refineError(error)
		const relativePath = error.fileName.replace(CHAR_DIRECTORY + '/', '')
