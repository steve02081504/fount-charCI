...
if not errorlevel 1 (
	pwsh.exe -noprofile -executionpolicy bypass -file "%~dp0fount-charCI.ps1" %*
	goto :exit_batch
)

where powershell.exe >nul 2>&1
if not errorlevel 1 (
	powershell.exe -noprofile -executionpolicy bypass -file "%~dp0fount-charCI.ps1" %*
	goto :exit_batch
)

if exist "C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe" (
	"C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe" -noprofile -executionpolicy bypass -file "%~dp0fount-charCI.ps1" %*
	goto :exit_batch
)

if exist "C:\Windows\SysWOW64\WindowsPowerShell\v1.0\powershell.exe" (
	"C:\Windows\SysWOW64\WindowsPowerShell\v1.0\powershell.exe" -noprofile -executionpolicy bypass -file "%~dp0fount-charCI.ps1" %*
	goto :exit_batch
)
...
if exist "%PowerShellExe64%" (
	"%PowerShellExe64%" -noprofile -executionpolicy bypass -file "%~dp0fount-charCI.ps1" %*
	goto :exit_batch
)
...
if exist "%PowerShellExe32%" (
	"%PowerShellExe32%" -noprofile -executionpolicy bypass -file "%~dp0fount-charCI.ps1" %*
	goto :exit_batch
)

for /f "delims=" %%i in ('where powershell.exe 2^>nul') do set PowerShellFullPathWhere=%%i
if defined PowerShellFullPathWhere (
	"%PowerShellFullPathWhere%" -noprofile -executionpolicy bypass -file "%~dp0fount-charCI.ps1" %*
	goto :exit_batch
)
